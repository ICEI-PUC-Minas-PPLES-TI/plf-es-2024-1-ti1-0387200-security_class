...
// Função assíncrona para buscar informações do curso no JSON Server
...
async function criarDiv(id) {
  try {
    ...
    const curso = await buscarInformacoesDoCurso(id);
...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    const informacoes = await carregarEExtrairJSON('http://localhost:3000/courses');

    if (informacoes && informacoes.length > 0) {
      informacoes.forEach(curso => {
        criarDiv(curso.id);
      });
    } else {
